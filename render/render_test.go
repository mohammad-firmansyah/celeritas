...
var pageData = []struct {
	...
	{"go_page_no_template", "go", "non-existent", true, "no error rendering non-existent go template, when one is expected"},
	{"jet_page", "jet", "home", false, "error rendering jet template"},
	{"jet_page_no_template", "jet", "non-existent", true, "no error rendering non-existent jet template, when one is expected"},
...
func TestRender_GoPage(t *testing.T) {
	...
	if err == nil {
		t.Error("Error rendering page non-existent ", err)
	}
}
